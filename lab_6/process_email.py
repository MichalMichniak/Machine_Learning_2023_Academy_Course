...
def process_email(email_contents: str) -> List[int]:
    ...
    vocabulary_dict_inverse = { value : key for key,value in vocabulary_dict.items()}
    ...
    email_contents = re.sub('(http://|https://)[a-zA-Z0-9]\S*', 'httpaddr ', email_contents)
    ...
    for token in tokens:
        ...
        try:
            word_indices.append(vocabulary_dict_inverse[token])
        except:
            continue
...
#         data = file.read()
#     return data
# print(process_email(read_file("./data/emailSample1.txt")))